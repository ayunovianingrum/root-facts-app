...
export class DetectionService {
  ...
  async loadModel() {
    try {
      const backend = (await isWebGPUSupported()) ? 'webgpu' : 'webgl';
      ...
      if (!validateModelMetadata(metadata)) {
        throw new Error('Invalid Metadata: Label array not found');
      }
      ...
      logError('Unable to load model', error);
  ...
  async predict(imageElement) {
    ...
    if (!imageElement) {
      throw new Error(
        'Failed to load image. Prediction requires an image input.',
      );
    }
